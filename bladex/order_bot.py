...
def fast_login(page):
    ...
    try:
        page.goto(
            "https://www.sen-sen.com.tw/account",
            wait_until="domcontentloaded",
            timeout=PAGE_TIMEOUT,
        )

        if "/account/login" not in page.url:
            print("[登入] 已是登入狀態，跳過登入")
            return True

    except Exception:
        pass

    print("[登入] 尚未登入，開始自動登入...")
    ...
    try:
        ...
        page.fill("input#login", username, timeout=1500)
        page.fill("input#password", password, timeout=1500)

        page.locator("button.submit[type='submit']").first.click(
            force=True,
            timeout=3000
        )
